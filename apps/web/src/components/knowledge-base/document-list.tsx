...
export function DocumentList({ knowledgeBaseId, onForceOcr }: DocumentListProps) {
            ...
            <div className="flex shrink-0 items-center gap-2">
              ...
              {doc.status === 'READY' && (
                <Button
                  ...
                  aria-label={previewDocId === doc.id ? 'Hide extracted text' : 'Preview extracted text'}
                >
                  {previewDocId === doc.id ? (
          ...
          {previewDocId === doc.id && (
            <div className="border-t border-border bg-muted/30 px-4 py-3">
              ...
              <DocumentPreview
                ...
                docId={doc.id}
                onReprocessWithOcr={
                  doc.fileType === 'PDF' && onForceOcr
                    ? () => {
                        if (
                          confirm(
                            `Delete "${doc.filename}" and re-upload with OCR enabled?\n\nYou will need to select the file again.`,
                          )
                        ) {
                          deleteMutation.mutate(doc.id)
